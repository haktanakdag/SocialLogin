...
class clsForms {
			...
			foreach ($veriler as $veri){
				...
				if ($dumeler<>""){
					...
					//if($dume[0]=="duzenle" or $dume[0]=="yeniekle" ) $onclick = str_replace("/", '"',"onclick=/return hs.htmlExpand(this, { objectType: 'ajax' } )/");
						echo "<a href='$dume[1]&$dume[0]=$veri->id' class='btn' $onclick> $dume[2] </a>";
					}
					echo "</td>";
				}
				echo "</tr>";
	...
	if ($ozellikler[2]==""){ $divbitis="</div>";} else { $divbitis="";}
		echo "$divbaslangic<input type='text' name='$ozellikler[0]' id='$ozellikler[0]' value='$ozellikler[1]' class ='$class' placeholder='$ozellikler[3]' >$divbitis";
		echo '<script>$(function(){$( "#'.$ozellikler[0].'" ).datepicker();});</script>';
	}
	...
	public function doRadioGroup($ozellikler){
		...
		echo "<input type='radio' id='$ozellikler[0]' name='$ozellikler[0]' value='$ozellikler[1]' $ozellikler[3]> $isim";
	}
